-- Fix #9: Entity document_number uniqueness should be scoped to document_type.
-- A CE and Pasaporte with the same alphanumeric value are different documents.
DROP INDEX IF EXISTS idx_entities_document_number_active;
CREATE UNIQUE INDEX idx_entities_document_number_active
  ON entities (document_type, document_number)
  WHERE is_active = true;

-- Fix #11: Move BDN-aware payment_status logic from app layer into v_invoice_balances.
-- An invoice is only 'paid' when total payments >= total AND detraccion is fully paid.
...
-- Recreate v_invoice_balances with BDN-aware payment_status
...
-- Recreate v_invoices_with_loans
...
-- Recreate v_obligation_calendar
