import { isCompanyView, getPartnerCompanyId } from '@/lib/auth'
...
export default async function PLPage({ searchParams }: Props) {
  ...
  const partnerCompanyId = await getPartnerCompanyId()
  ...
  const [data, exchangeRate] = await Promise.all([
    getCompanyPL(periodMode, year, quarter, month, 'PEN', isAlex, partnerCompanyId),
