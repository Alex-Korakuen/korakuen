    ...
    <div className="rounded-lg border-2 border-negative/20 bg-negative-bg px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h4 className="text-sm font-semibold text-negative">{title}</h4>
          <p className="text-sm text-negative mt-1">{message}</p>
          <p className="text-xs text-negative mt-2">This action can be reversed by an administrator.</p>

          {error && <p className="text-xs font-medium text-negative mt-2">{error}</p>}
