...
export async function registerDirectTransaction(input: {
  partner_company_id: string
  ...
  if (input.amount <= 0) return { error: 'Amount must be greater than 0' }
  if (!input.project_id) return { error: 'Project is required' }
  if (!input.partner_company_id) return { error: 'Partner is required' }

  const invoiceDirection = input.direction === 'outflow' ? 'payable' : 'receivable'
  const paymentDirection = input.direction === 'outflow' ? 'outbound' : 'inbound'
  const costType = input.direction === 'outflow' ? 'project_cost' : null
  const title = `Direct transaction — ${input.date}`
  ...
  if (invError) return { error: handleDbError(invError, 'Failed to create invoice') }
  ...
  if (itemError) {
    ...
    return { error: handleDbError(itemError, 'Failed to create invoice item') }
  }

  // 3. Create payment marking it as fully paid
  ...
  if (pmtError) return { error: handleDbError(pmtError, 'Failed to register payment') }
